 ...
 * 
 * 
 * 作用: 管理变量表，并对变量的格式进行约束
 *
 * 成员：
 *       list     	:JList<String>				列表控件，列出变量表
 *       listModel	:DefaultListModel<String>	管理变量内容
 *       editButton	:JButton					按钮控件，点击以编辑变量
 *       addButton	:JButton					按钮控件，点击以添加变量
 *       delButton	:JButton					按钮控件，点击以删除变量
 *
 *       
	...
	public Variable() {
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String variable = JOptionPane.showInputDialog("输入新变量").trim();
				//检查是否已存在
				if (listModel.contains(variable)) {
					JOptionPane.showMessageDialog(null, "变量" + variable + "已存在",
							"变量重复", JOptionPane.ERROR_MESSAGE);
				}
				//检查格式是否正确
				else if (!isVariableFormat(variable)) {
					JOptionPane.showMessageDialog(null, "变量" + variable + "格式错误",
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (index < 0) {
					JOptionPane.showMessageDialog(null, "未选定任何变量",
